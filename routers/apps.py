...
# ============================================================
# ПРОДВИНУТЫЙ ПОИСК (как в оригинале)
# ============================================================

def _search_score(app: dict, query: str) -> int:
    """
    Вычисляет релевантность поиска по нескольким полям с весами.
    Чем больше score — тем выше в результатах.
    """
    q = query.lower()
...
async def search_apps(
    ...
    """
    Продвинутый поиск по всем полям приложения с релевантностью.
    Сортирует: сначала совпадения в названии, потом авторе, пакете, категории, тегах, описании.
    """
    # Сначала получаем все приложения (с фильтрами по категории/типу если указаны)
    ...
    # Сортируем по score (убывание), потом по downloads, потом по названию
...
# ============================================================
# СКАЧИВАНИЕ — последняя версия и конкретная версия
# ============================================================
...
async def download_apk_latest(app_id: int, request: Request):
    ...
    # Сначала ищем apk_file напрямую, потом в versions
...
async def download_apk_version(app_id: int, version_code: str, request: Request):
    """Скачать конкретную версию приложения по version_code."""
    row = await fetch_one("SELECT data FROM apps WHERE id = ?", (app_id,))
    if not row:
        raise HTTPException(status_code=404)
    ...
    # Для конкретной версии тоже считаем скачивание
    ip = get_real_ip(request)
    await record_download_once(app_id, ip)
