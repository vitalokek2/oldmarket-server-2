...
async def download_apk(app_id: int, request: Request):
    row = await fetch_one("SELECT data FROM apps WHERE id = ?", (app_id,))
    if not row:
        raise HTTPException(status_code=404)
    data = json.loads(row["data"])
    apk_name = data.get("apk_file") or (
        data.get("versions", [{}])[0].get("apk_file") if data.get("versions") else None
    )
    if not apk_name:
    ...
    # Считаем уникальное скачивание по IP, чтобы счётчик не накручивался
    # повторными запросами одного и того же человека (как делал оригинал).
    ip = get_real_ip(request)
    await record_download_once(app_id, ip)

    return FileResponse(
        path=file_path, filename=apk_name, media_type="application/vnd.android.package-archive"
    )
